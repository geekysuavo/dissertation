...
\figref{2.5}{Figure 2.5} show the deterministic schedules resulting
...
\figref{2.6}{Figure 2.6} compares the true HSQC spectrum with
...
\figref{2.7}{Figure 2.7} compares \hcnmr{} projections of the true HNCA with
...
density in \figref{2.5}{Figure 2.5}. For one-dimensional schedules,
...
analysis was also performed (cf. \figref{2.8}{Figure 2.8}) to ensure
convergence of IST to a stationary point, as measured by a lack of decrease
in the $\ell_2$ error. \figref{2.9}{Figure 2.9A} shows the distributions of
...
\begin{doublespace}
A summary of the relative HSQC peak-picking performance for the IST
reconstructions from each sampling schedule and at each sampling density is
listed in Table 2.1. For each 2D \hnnmr{} HSQC spectrum of ubiquitin
reconstructed via IST at each sampling density and each sampling method,
a set of quality statistics was computed. Peak lists were generated using the
...
computed. Identical procedures and parameters, with the exception of an
intensity threshold of $6.0 \times 10^8$, were used to peak-pick the \hnnmr{}
projections of the uniform and reconstructed HNCA spectra (cf. Table 2.2).
\end{doublespace}
...
computed (\figref{2.10}{Figure 2.10}). If the two methods were indeed
equivalent, one would expect the histograms in \figref{2.10}{Figure 2.10A}
to resemble Poisson distributions (\figref{2.10}{2.10B}).
\end{doublespace}
...
(\figref{2.7}{Figure 2.7}) also reveal a high similarity between the
...
into the sine-burst point-spread function (\figref{2.11}{Figure 2.11}) by
...
IST residuals of sine-burst schedules (\figref{2.9}{Figure 2.9}, dashed lines)
...
(\figref{2.7}{Figure 2.7}), where the more incoherent sine-burst schedule
...
spectra (\figref{2.7}{Figure 2.7}) after IST reconstruction, even at low
