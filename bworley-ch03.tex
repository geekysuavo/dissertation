...
\ENSURE $\mathbf{t} \in \mathbb{R}^N$,%
      ...
      $\:\mathbf{w_b} \in \mathbb{R}^K
       \quad \forall b \in \{1, 2, \dots B\}$
\STATE $\mathbf{X} \gets [\mathbf{X_1}, \; \dots \mathbf{X_B}]$
\STATE $\mathbf{u} \sim U_{N \times 1}$ \COMMENT{
  $\mathbf{u}$ may also be initialized to a column of $\mathbf{Y}$
}
\REPEAT
...
$n$ data blocks, was developed \cite{lofstedt2012}. Like in O2PLS, no matrix
in OnPLS is special, and all matrices are regressed against all others in a
complete association graph (Figure 3.11A). While such complete connectivity
...
the proposed multiblock OPLS (MB-OPLS) method is a more suitable candidate.
\\\\
Multiblock OPLS decomposes each block $\mathbf{X_b}$ into a set of
...
\mathbf{X} =& \: [\mathbf{X_1}, \; \mathbf{X_2}, \; \dots \mathbf{X_B}] \\
           =& \: \mathbf{T} \mathbf{P}^T +
                 \mathbf{T_o} \mathbf{P_o}^T + \mathbf{E} \\
\mathbf{X_b} =& \: \mathbf{T_b} \mathbf{P_b}^T +
                   \mathbf{T_{b,o}} \mathbf{P_{b,o}}^T + \mathbf{E_b}
 \quad \forall b \in \{1, 2, \dots B\} \\
\mathbf{Y} =& \: \mathbf{U} \mathbf{C}^T + \mathbf{G} \\
           =& \: \mathbf{T} \mathbf{C}^T + \mathbf{F} \nonumber
\end{align}
...
\begin{doublespace}
The proposed MB-OPLS algorithm is based on NIPALS MB-PLS, with an added
OPLS-type OSC filter that removes $\mathbf{Y}$-uncorrelated variation from
super-loadings. Deflation of predictive and orthogonal components from each
block is achieved using super-scores, as in the above MB-PLS algorithm.
\end{doublespace}
...
\ENSURE $\mathbf{t} \in \mathbb{R}^N$,%
      ...
      $\:\mathbf{w_b} \in \mathbb{R}^K
       \quad \forall b \in \{1, \dots B\}$ \\
      $\:\mathbf{T_{b,o}} \in \mathbb{R}^{N \times a}$,%
      $\:\mathbf{P_{b,o}} \in \mathbb{R}^{K \times a}$,%
      $\:\mathbf{W_{b,o}} \in \mathbb{R}^{K \times a}
       \quad \forall b \in \{1, \dots B\}$
\STATE $\mathbf{X} \gets [\mathbf{X_1}, \dots \mathbf{X_B}]$
\STATE $\mathbf{u} \sim U_{N \times 1}$ \COMMENT{
  $\mathbf{u}$ may also be initialized to a column of $\mathbf{Y}$
...
\STATE $a \gets 0$
\STATE $\mathbf{E} \gets \mathbf{X}$
\STATE $\mathbf{E_b} \gets \mathbf{X_b}
        \quad \forall b \in \{1, \dots B\}$
\WHILE{\NOT done}
  \REPEAT
    \FORALL{$b \in \{1, \dots B\}$}
      \STATE $\mathbf{w_b} \propto \mathbf{E_b}^T \mathbf{u}$
      \STATE $\mathbf{t_b} \gets \mathbf{E_b} \mathbf{w_b}$
      \STATE $\mathbf{p_b} \gets \tfrac{\mathbf{E_b}^T \mathbf{t_b}}
                                       {\mathbf{t_b}^T \mathbf{t_b}}$
    \ENDFOR
    \STATE $\mathbf{R} \gets [\mathbf{t_1}, \dots \mathbf{t_B}]$
    \STATE $\mathbf{w} \propto \mathbf{R}^T \mathbf{u}$
    \STATE $\mathbf{t} \gets \mathbf{R} \mathbf{w}$
  ...
  \STATE $\mathbf{p_o} \gets \tfrac{\mathbf{E}^T \mathbf{t_o}}
                                   {\mathbf{t_o}^T \mathbf{t_o}}$
  \STATE $\mathbf{p_{b,o}} \gets \tfrac{\mathbf{E_b}^T \mathbf{t_o}}
                                       {\mathbf{t_o}^T \mathbf{t_o}}
          \quad \forall b \in \{1, \dots B\}$
  \STATE $\lambda \gets \tfrac{\| \mathbf{z} \|_2}{\| \mathbf{p} \|_2}$
  \IF{$\lambda < \lambda_{th}$}
    ...
    \STATE $\mathbf{W_o} \gets [\mathbf{W_o}, \; \mathbf{w_o}]$
    \STATE $\mathbf{E} \gets \mathbf{E} - \mathbf{t_o} \mathbf{p_o}^T$
    \STATE $\mathbf{E_b} \gets \mathbf{E_b} - \mathbf{t_o} \mathbf{p_{b,o}}^T
            \quad \forall b \in \{1, \dots B\}$
    \STATE $a \gets a + 1$
  \ELSE
    \STATE done $\gets$ \TRUE
...
\begin{figure}[ht!]
\begin{center}
  \includegraphics[width=5.5in]{figs/mva/11.png}
\end{center}
\caption
  ...
  models, where each data block $\mathbf{X_b}$ predicts a set of responses in
