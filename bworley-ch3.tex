...
free induction decay tensor is multiplied pointwise by a window function
...
multiplied pointwise with the data to correct for phase errors
...
when the ultimate fast of the spectral data is multivariate analysis, the
...
suffer from imprecisions in \hnmr{} chemical shifts between observations.
...
directly on full-resolution spectral data in $\mathbf{X}$. The traditional
method of mitigating imperfect variable correspondence in a data matrix is
to partition the original set of variables into a smaller set of regions,
referred to as ``bins'', and to integrate each bin to yield a data matrix
having reduced dimensionality.
\\\\
While binning masks variable mis-correspondence, filters incoherent spectral
noise, and achieves substantial dimensionality reduction, it often hides
potentially significant variation in low-intensity resonances nearby
...
integrated together in the process. When full spectral resolution is required
during model training and interpretation, the correspondence problem in NMR
and chromatographic datasets may be alternatively addressed by signal alignment
methods.
\\\\
...
for normalizing data tensors produced by NMR and other spectroscopies
\cite{fearn:cils2009}.
\end{doublespace}
