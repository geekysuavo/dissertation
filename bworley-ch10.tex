...
\caption
  ...
  experimental data (Figure 10.2) is overlaid on the
...
range of structural parameters (Figure 10.1A) corresponding roughly to the
...
\caption
  ...
  to generate a transparency mask for Figure 10.1.
}
\end{SCfigure}
...
an \npistar{} interaction were labeled as interactors (Figure 10.2). Standard
...
Figure 10.3 illustrates the relationship between $V_{dd}$ and \cnmr{}
...
$\theta = 90^\circ$ is shown in Figure 10.4. Finally, a full set of shielding
...
$d$ and $\theta$ values for each carbonyl pair (Figure 10.1B). These chemical
...
residues are near the twisted $\beta$-sheet region (Figure 10.5). Significant
...
in Figure 10.3. The dipole-dipole potentials were calculated using the
...
Figure 10.3 with the largest \cnmr{} chemical shift difference
...
from Figure 10.6 that any contribution from a hydrogen bond to the \cnmr{}
...
Figure 10.3 with low \cnmr{} chemical shifts and negative dipole-dipole
...
illustrated in Figure 10.1A. Clearly, $d$ and $\theta$ values alone fail to
...
in Figure 10.4, for $d$ = 2.9 \r{A} and $\theta = 100^\circ$. The computed
...
Figure 10.7B. This energy was computed using a very simple model assuming the
...
chemical shielding surface (Figure 10.8A). The residual shows a
...
bond distance $d_{O-N}$ (see Supplemental Figure 10.8B). In accordance with
...
a possible \nspistar{} interaction. As can be seen in Figure 10.9, for values
...
This is depicted in Figure 10.4A for the Hartree-Fock HOMO of the hydrogen bond
